...
* Fixed: the reference index and `llms.txt` now link to a topic's page by its
  ...
  name with no summary ([#80](https://github.com/etiennebacher/altdoc/issues/80)).
...
* Fixed: the message `render_docs()` prints after writing the reference index
  ...
  entries than the page lists ([#80](https://github.com/etiennebacher/altdoc/issues/80)).
...
* Added: `check_altdoc()` reports configuration problems without rendering.
  ...
  navigation.
  The function is opt-in: `render_docs()` does not call it ([#39](https://github.com/etiennebacher/altdoc/issues/39)).
...
* Internal: the render tests now assert which files each generator publishes,
  ...
  from being written ([#79](https://github.com/etiennebacher/altdoc/issues/79)).
...
* Added: `render_docs()` now writes an [`llms.txt`](https://llmstxt.org/) at
  ...
  `altdoc/reference.yml` ([#40](https://github.com/etiennebacher/altdoc/issues/40)).
...
* Added: man pages can now be labeled in the sidebar with their `.Rd`
  ...
  asks for it ([#37](https://github.com/etiennebacher/altdoc/issues/37)).
...
* Fixed: `render_docs()` now imports `CONTRIBUTING.md` to `docs/`, so the
  ...
  automatically, so add a `$ALTDOC_CONTRIBUTING` entry to pick it up ([#4](https://github.com/etiennebacher/altdoc/issues/4)).
...
* Fixed: a `docute` vignette title containing a backslash no longer produces a
  ...
  enclose it, but not for the backslash that does the escaping ([#37](https://github.com/etiennebacher/altdoc/issues/37)).

* Internal: removed unused `filename` parameter from `.substitute_altdoc_variables()` ([#44](https://github.com/etiennebacher/altdoc/issues/44)).
...
* Added: `render_docs()` now generates a reference index at `reference.md`,
  ...
  pick it up ([#33](https://github.com/etiennebacher/altdoc/issues/33), [#46](https://github.com/etiennebacher/altdoc/issues/46)).
...
* Fixed: for `quarto_website` sites with `code-link: true`, `downlit`'s
  ...
  CRAN packages, so for a non-CRAN package every such link was dead ([#25](https://github.com/etiennebacher/altdoc/issues/25)).
...
* Fixed: for `quarto_website` sites with `code-link: true`, links to the
  ...
  commonly a pull-request preview served under its own subpath ([#10](https://github.com/etiennebacher/altdoc/issues/10)).
...
* Vignettes in subfolders (e.g., `vignettes/articles/`) are now discovered 
  recursively when using `quarto_website`. ([#355](https://github.com/etiennebacher/altdoc/issues/355))

* Custom sidebars with singleton entries in `quarto_website.yml` are now 
  properly processed. ([#355](https://github.com/etiennebacher/altdoc/issues/355))
...
* In 0.5.0, we announced that `README.qmd` wouldn't be automatically rendered,
  ...
  meaning that `altdoc` uses `README.md` over `README.qmd` ([#354](https://github.com/etiennebacher/altdoc/issues/354)).

* Figures stored in `man/figures` and used in the README are now properly
  displayed ([#354](https://github.com/etiennebacher/altdoc/issues/354)).

* Fix for change of default value of `getOption("help.htmltoc")` in R-devel
  4.6.0 ([#356](https://github.com/etiennebacher/altdoc/issues/356)).
...
* When using `mkdocs` as documentation generator, the Python virtual environment
  ...
  doesn't have to be `.venv_altdoc` located at the project root anymore ([#339](https://github.com/etiennebacher/altdoc/issues/339)).
...
* `render_docs()` now updates the file `altdoc/pkgdown.yml`. This file was also
  ...
  website ([#344](https://github.com/etiennebacher/altdoc/issues/344)).
...
* The CSS for arguments table in "Reference" page has changed a bit. Argument
  ...
  readability in case of multiple arguments on the same line ([#308](https://github.com/etiennebacher/altdoc/issues/308)).
...
* `render_docs()` now shows an explicit error message if Quarto is not installed
  on the system ([#329](https://github.com/etiennebacher/altdoc/issues/329)).

* Fix an error in `render_docs()` when the title in the Rd file is split across
  several lines ([#333](https://github.com/etiennebacher/altdoc/issues/333)).
