.rd2qmd <- function(source_file, target_dir, path) {
    ...
    if (!is.na(h2_start) && !is.na(h2_end) && h2_start <= h2_end) {
        title_lines <- tmp[h2_start:h2_end]
        title <- paste(title_lines, collapse = " ")
        title <- gsub("<h2[^>]*>(.*)</h2>", "## \\1 {.unnumbered}\n", title)
        title <- gsub("\u201c|\u201d", "\"", title)
        before <- if (h2_start > 1) tmp[seq_len(h2_start - 1)] else character(0)
        after <- if (h2_end < length(tmp)) tmp[(h2_end + 1):length(tmp)] else character(0)
        tmp <- c(before, title, after)
    }
