...
#'
#' * `docs/README.md` (or `_quarto/README.qmd` for Quarto websites)
#'   - README.md (or README.qmd when using Quarto website and `README.qmd` is present)
#'   - Note: For `quarto_website`, if `README.qmd` is present in the package root, it is copied to `_quarto/README.qmd` and included via `index.qmd`. For other generators, `README.md` is required. `README.qmd` and `README.Rmd` are not rendered automatically for docute, docsify, or mkdocs, so keep `README.md` in step with whichever of them you write in for those generators.
#' * `docs/NEWS.md`
#'   - NEWS
