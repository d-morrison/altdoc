...
#' * `docs/README.md`
#'   - README.md
#'   - Note: `README.md` is required, and is always copied. For `quarto_website`, if `README.qmd` is present, it is copied and included directly in `index.qmd`. For other generators, `README.qmd` and `README.Rmd` are not rendered for you, so keep `README.md` in step with whichever of them you write in. When more than one is present, the first of `README.qmd`, `README.Rmd`, `README.md` is the one the `freeze` check watches for changes.
#' * `docs/NEWS.md`
#'   - NEWS
