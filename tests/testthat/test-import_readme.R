...
test_that("quarto_website uses index.qmd when README.qmd is present (#368)", {
    dir <- local_readme_package(
        "README.md" = "# readme",
    ...
    expect_false(any(grepl("does not render README", msgs)))
    expect_true(any(grepl("imported", msgs)))
...
test_that("quarto_website uses index.md when README.qmd is absent", {
    dir <- local_readme_package(
        "README.md" = "# readme"
    )
    withr::local_dir(dir)
